...
namespace Demo
{
    ...
    {
        public int buildingHeight; // The total building height (=#stocks) - value should be the same for all stocks
        ...
        GameObject SpawnPrefabWithRandomRotation(GameObject prefab)
        {
            // Generate a random rotation around the y-axis
            Quaternion randomRotation = Quaternion.Euler(0, Random.Range(0, 360), 0);
            return Instantiate(prefab, transform.position, randomRotation);
        }
    }
}
